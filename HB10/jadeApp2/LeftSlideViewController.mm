...
- (void)viewDidLoad {
    ...
    //    添加通知
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(openLeftView) name:RootNavSwipNot object:nil];
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(closeLeftView) name:RootNavMenuHiden object:nil];
}
 ...
 */
- (instancetype)initWithLeftView:(RootViewController *)leftVC
                     andMainView:(UINavigationController *)mainVC
{
    if(self = [super init]){
        ...
        leftVC.automaticallyAdjustsScrollViewInsets = NO;
        mainVC.automaticallyAdjustsScrollViewInsets = NO;
        //滑动手势
...
//    [self.leftVC setATableHeardView];
...
-(BOOL)gestureRecognizer:(UIGestureRecognizer*)gestureRecognizer shouldReceiveTouch:(UITouch*)touch {
    ...
    if(touch.view.tag == vDeckCanNotPanViewTag ||(point.x > kScreenWidth*0.3 && self.mainVC.view.x==0) )
    {
//        NSLog(@"不响应侧滑");  当处于覆盖状态 并且点击的位置大于 1/3 的宽度  那么不相应侧滑
         return NO;
    }
    else
    {
