...
/**
 设置设备报警时推送的绑定账号
 @param accounts                要绑定的账号数组(多个账号),参数为字符串,绑定成功后覆盖原来的数据
 @param deviceID                设备ID
 @param devicePassword          设备密码
 @param completionBlock         与设备交互完成后的回调Block
 */
...
/**
 ...
 @"email is verified"  邮箱是否验证，类型：BOOL
...
/**
 设置设备报警邮箱:报警时,若完整地设置邮箱(SMTP及密码都通过验证),用设置的邮箱发送邮件至设置的邮箱;若不完整设置,则用技威服务器的邮箱发送邮件至设置邮箱
